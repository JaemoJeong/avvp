...
def compute_confusion_matrix(all_gt, all_pred, categories):
    """
    all_gt: (N_samples * 10, num_classes) - 모든 샘플의 세그먼트별 GT
    all_pred: (N_samples * 10, num_classes) - 모든 샘플의 세그먼트별 예측값
    """
    num_classes = len(categories)
    # 실제(Row) -> 예측(Col) 행렬
    conf_mat = np.zeros((num_classes, num_classes))

    # 세그먼트별로 루프를 돌며 GT가 1인 지점을 찾음
    for i in range(all_gt.shape[0]):
        gt_indices = np.where(all_gt[i] == 1)[0]
        ...
        for gt_idx in gt_indices:
            if len(pred_indices) > 0:
                ...
                # 모델이 아무것도 예측하지 못한 경우 (Optional: 분석용)
                pass 

    # 시각화를 위해 정규화 (Row 기준: 해당 클래스 등장 대비 예측 비율)
    conf_mat_norm = conf_mat / (conf_mat.sum(axis=1, keepdims=True) + 1e-9)
...
def calculate_metrices_LLP(video_dir_path, pred, categories, split="test", get_per_class=False):
    ...
    df = df[df['filename'].apply(lambda x: '_'.join(x.split('_')[:-2])).isin(predicted_video_ids)]
    df_a = df_a[df_a['filename'].apply(lambda x: '_'.join(x.split('_')[:-2])).isin(predicted_video_ids)]
    df_v = df_v[df_v['filename'].apply(lambda x: '_'.join(x.split('_')[:-2])).isin(predicted_video_ids)]
    print(f"Evaluating on {len(df)} samples (filtered to match predictions)")
    ...
    for file_name in df["filename"].values:
        ...
        # 2. 루프 내부 (for file_name in df["filename"].values:)
        # ... GT_av와 SO_av 계산 후 ...
        all_gt_av.append(GT_av.T) # (10, 25)
        all_so_av.append(SO_av.T) # (10, 25)
    ...
    for label in categories:
        ...
        sample_count = len(metrices_per_class[label]['F_seg_av'])
        print(f"{label}: {sample_count} samples processed")
    ...
    print(f"downloaded: {downloaded}, not_downloaded: {not_downloaded}")

    # 3. 루프 종료 후
    all_gt_av = np.concatenate(all_gt_av, axis=0) # (Total_segments, 25)
    all_so_av = np.concatenate(all_so_av, axis=0)
    cm = compute_confusion_matrix(all_gt_av, all_so_av, categories)

    def plot_confusion_matrix(cm, categories, save_path):
        # 저장 디렉토리가 없으면 생성
        ...
        plt.title('Audio-Visual Event Parsing Confusion Matrix (Normalized)', fontsize=15)
        
        # 파일 저장
        file_full_path = os.path.join(save_path, "confusion_matrix_av.png")
        plt.savefig(file_full_path, bbox_inches='tight', dpi=300)
        print(f"✅ Confusion Matrix saved to: {file_full_path}")
        plt.show()
        plt.close()

    # 실행
    plot_confusion_matrix(cm, categories, save_path="./analysis/confusion_matrix")
